...
# de_quack_project R wrapper for the local Python de_quack library.
# This wrapper uses reticulate to expose core.py and viz.py functions from R.

if (!requireNamespace("reticulate", quietly = TRUE)) {
  stop("Please install the 'reticulate' package to use de_quack from R.")
}
if (!requireNamespace("R6", quietly = TRUE)) {
  stop("Please install the 'R6' package to use the de_quack wrapper.")
}

resolve_de_quack_path <- function(python_path = NULL) {
  ...
  if (reticulate::py_module_available("de_quack")) {
    py_env <- reticulate::import("de_quack")
  ...
  local_candidates <- c(
    file.path(getwd(), "de_quack"),
    file.path(getwd(), "de_quack_project", "de_quack"),
    getwd()
  )
  ...
  stop("de_quack Python module not found. Please run 'pip install de_quack' or provide python_path.")
}

load_de_quack_modules <- function(python_path = NULL) {
  python_path <- resolve_de_quack_path(python_path)
  message("Loading de_quack Python modules from: ", python_path)
...
DeDuck <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(python_path = NULL, db_path = "SQL.duckdb", use_condaenv = NULL, use_virtualenv = NULL) {
      ...
      modules <- load_de_quack_modules(python_path)
    ...
    connect = function(db_path = NULL) {
      ...
      self$py_db <- self$py_core$de_quackling(db_path = target_path)
      self$py_db$connect()
      invisible(self)
    },
...
de_quack <- function(python_path = NULL, db_path = "SQL.duckdb", use_condaenv = NULL, use_virtualenv = NULL) {
  DeQuack$new(
